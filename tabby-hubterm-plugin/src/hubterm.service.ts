...
export class HubTermService {
    ...
    attachTab (tab: BaseTerminalTabComponent): void {
        this.attachedTabs.set(tab, this.sessionFromTab(tab))
    ...
    private collectSessions (): SessionInfo[] {
        ...
        for (const tab of this.attachedTabs.keys()) {
            const session = this.sessionFromTab(tab)
    ...
    private sessionFromTab (tab: BaseTerminalTabComponent): SessionInfo {
        ...
        const host = this.cleanString(options.host)
        const port = Number(options.port || 22)
        const user = this.cleanString(options.user)
        const displayName = this.cleanString(profile?.name || tabState.sessionName || tabState.title)

        if (host) {
            return {
                session_id: tabState.hubtermSessionId,
                display_name: displayName || `${user ? `${user}@` : ''}${host}`,
                port_name: `${host}:${Number.isFinite(port) && port > 0 ? port : 22}`,
                user,
                type: existing?.type || 'master',
                client_ip: existing?.client_ip || 'tabby',
                connected_at: existing?.connected_at || Math.floor(Date.now() / 1000),
            }
        }
        ...
        return {
            ...
            port_name: this.cleanString(tabState.sessionName || profile?.name) || 'terminal',
            user: user || '',
            type: existing?.type || 'master',
            client_ip: existing?.client_ip || 'tabby',
            connected_at: existing?.connected_at || Math.floor(Date.now() / 1000),
    ...
    private tabForSession (sessionId: string): BaseTerminalTabComponent | null {
